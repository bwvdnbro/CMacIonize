...
def spitzer(t):
  global Rst, ci
  return Rst*(1. + 1.75*ci*t/Rst)**(4./7.)
...
def hosokawa_inutsuka(t):
  global Rst, ci
  return Rst*(1. + 1.75*np.sqrt(4./3.)*ci*t/Rst)**(4./7.)

# get the reference curves
trange = np.arange(0., 0.15*Myr, 0.001*Myr)
...
for f in sorted(glob.glob("starbench_voronoi_*.hdf5")):
  ...
  ax[0].plot(radius/pc, rho * 1.e-3, "k.")
  ax[0].set_ylim(0., 3.e-20)
...
pl.plot(trange/Myr, hosinutrange/pc, "g-", label = "Hosokawa-Inutsuka solution")
