...
int main(int argc, char **argv) {
  ...
  assert_condition(index == 8 * 16 * 16 + 8 * 16 + 8);
  ...
  assert_condition(cell.get_sides().x() == 1. / 16.);
  assert_condition(cell.get_sides().y() == 1. / 16.);
  assert_condition(cell.get_sides().z() == 1. / 16.);

  CoordinateVector<> cell_top_anchor = cell.get_top_anchor();

  assert_condition(cell_top_anchor.x() == 0.5 + 1. / 16.);
  assert_condition(cell_top_anchor.y() == 0.5 + 1. / 16.);
  assert_condition(cell_top_anchor.z() == 0.5 + 1. / 16.);
  ...
  assert_condition(midpoint.y() == 0.5 + 1. / 32.);
  assert_condition(midpoint.z() == 0.5 + 1. / 32.);
  ...
  assert_condition(surface_area == (1. / 16.) * (1. / 16.));

  ngbindexexp =
      DensityGrid::iterator(
          grid.get_cell_index(CoordinateVector<>(0.6, 0.51, 0.51)), grid)
          .get_index();
  ngbindex = std::get< 0 >(ngbs[1]).get_index();
  assert_condition(ngbindex == ngbindexexp);
  midpoint = std::get< 1 >(ngbs[1]);
  assert_condition(midpoint.x() == 0.5 + 1. / 16.);
  assert_condition(midpoint.y() == 0.5 + 1. / 32.);
  assert_condition(midpoint.z() == 0.5 + 1. / 32.);
  ...
  assert_condition(midpoint.x() == 0.5 + 1. / 32.);
  ...
  ngbindexexp =
      DensityGrid::iterator(
          grid.get_cell_index(CoordinateVector<>(0.51, 0.6, 0.51)), grid)
          .get_index();
  ngbindex = std::get< 0 >(ngbs[3]).get_index();
  assert_condition(ngbindex == ngbindexexp);
  midpoint = std::get< 1 >(ngbs[3]);
  assert_condition(midpoint.x() == 0.5 + 1. / 32.);
  assert_condition(midpoint.y() == 0.5 + 1. / 16.);
  assert_condition(midpoint.z() == 0.5 + 1. / 32.);
  ...
  ngbindexexp =
      DensityGrid::iterator(
          grid.get_cell_index(CoordinateVector<>(0.51, 0.51, 0.6)), grid)
          .get_index();
  ...
  assert_condition(midpoint.x() == 0.5 + 1. / 32.);
  assert_condition(midpoint.y() == 0.5 + 1. / 32.);
  assert_condition(midpoint.z() == 0.5 + 1. / 16.);
  ...
  // check faces
  {
    ...
    assert_condition(faces[0].get_midpoint() ==
                     CoordinateVector<>(0.5, 0.5 + 1. / 32., 0.5 + 1. / 32.));
    {
      ...
      ++vertices;
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5, 0.5 + 1. / 16., 0.5));
      ++vertices;
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5, 0.5 + 1. / 16., 0.5 + 1. / 16.));
      ++vertices;
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5, 0.5, 0.5 + 1. / 16.));
      ++vertices;
      assert_condition(vertices == faces[0].last_vertex());
    }

    assert_condition(
        faces[1].get_midpoint() ==
        CoordinateVector<>(0.5 + 1. / 16., 0.5 + 1. / 32., 0.5 + 1. / 32.));
    {
      auto vertices = faces[1].first_vertex();
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5 + 1. / 16., 0.5, 0.5));
      ++vertices;
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5 + 1. / 16., 0.5 + 1. / 16., 0.5));
      ++vertices;
      assert_condition(
          vertices.get_position() ==
          CoordinateVector<>(0.5 + 1. / 16., 0.5 + 1. / 16., 0.5 + 1. / 16.));
      ++vertices;
      assert_condition(vertices.get_position() ==
                       CoordinateVector<>(0.5 + 1. / 16., 0.5, 0.5 + 1. / 16.));
      ++vertices;
    }

    assert_condition(faces[2].get_midpoint() ==
                     CoordinateVector<>(0.5 + 1. / 32., 0.5, 0.5 + 1. / 32.));
    {
    ...
    assert_condition(
        faces[3].get_midpoint() ==
        CoordinateVector<>(0.5 + 1. / 32., 0.5 + 1. / 16., 0.5 + 1. / 32.));
    {
    ...
    assert_condition(faces[4].get_midpoint() ==
                     CoordinateVector<>(0.5 + 1. / 32., 0.5 + 1. / 32., 0.5));
    {
    ...
    assert_condition(
        faces[5].get_midpoint() ==
        CoordinateVector<>(0.5 + 1. / 32., 0.5 + 1. / 32., 0.5 + 1. / 16.));
    {
  ...
  double ds;
  {
    ...
    assert_condition(intersection.x() == 0.5 + 1. / 16.);
    assert_condition(intersection.y() == 0.51);
    assert_condition(intersection.z() == 0.51);
    assert_values_equal(ds, 1. / 16. - 0.01);
  }
  {
  ...
  }
  {
    ...
    assert_condition(intersection.y() == 0.5 + 1. / 16.);
    assert_condition(intersection.z() == 0.51);
    assert_values_equal(ds, 1. / 16. - 0.01);
  }
  {
  ...
  }
  {
    ...
    assert_condition(intersection.x() == 0.51);
    assert_condition(intersection.y() == 0.51);
    assert_condition(intersection.z() == 0.5 + 1. / 16.);
    assert_values_equal(ds, 1. / 16. - 0.01);
  }
  {
  ...
  }
  {
    ...
    // the solution:
    //  l_x = 1./16.-0.01
    //  l_y = (1./16.-0.01)/2 = 0.0263
    //  l_z = 0.01/3 = 0.0033...
    // ==> z closest
    assert_condition(next_index.x() == 0);
    assert_condition(next_index.y() == 0);
    assert_condition(next_index.z() == -1);
    // \vec{i} = \vec{o} + l_z \vec{d}
    // i_x = 0.51 + 0.01/3.
    // i_y = 0.51 + 0.02/3.
    // i_z = 0.51 - 0.0084375
    assert_condition(intersection.x() == 0.51 + 0.01 / 3.);
    assert_condition(intersection.y() == 0.51 + 0.02 / 3.);
    assert_condition(intersection.z() == 0.5);
    assert_values_equal(ds, 0.0124722);
  }
  {
    ...
    assert_condition(intersection.y() == 0.5 + 1. / 16.);
    assert_condition(intersection.z() == 0.5 + 1. / 16.);
    assert_values_equal(ds, 0.0742462);
  }
  {
    ...
    assert_condition(intersection.y() == 0.5 + 1. / 16.);
    assert_condition(intersection.z() == 0.5 + 1. / 16.);
    assert_values_equal(ds, 0.0909327);
  }
