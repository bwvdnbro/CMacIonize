...
/**
 * @file testSurfaceDensityIonizedCalculator.cpp
 *
 * @brief Unit test for the SurfaceDensityIonizedCalculator class.
 *
...
int main(int argc, char **argv) {
  ...
  for (auto cellit = subgrid.hydro_begin(); cellit != subgrid.hydro_end();
    ...
    cellit.get_ionization_variables().set_ionic_fraction(ION_H_n, 0.5);
  }

  SurfaceDensityIonizedCalculator calculator(1, 32);
  calculator.calculate_surface_density(0, subgrid);
  calculator.output("test_surfaceDensityIonizedCalculator.txt", Box<>(0., 1.));
